...
Model: GPT-2 (Transformer-based language model)

Frameworks: PyTorch, Hugging Face Transformers, Flower (FL)
...
Evaluation Metrics: Training loss, perplexity, BLEU score, qualitative text inspection

A centralized baseline model was trained for comparison to evaluate whether federated fine-tuning impacts generative performance.

Results Summary

Experimental results demonstrate that the federated global model converges more consistently than individual client models across training rounds. The global model achieved:

Progressive reduction in training loss

Lower perplexity compared to the centralized baseline

Improved BLEU scores indicating better linguistic alignment

Medically coherent and contextually relevant generated text

These results indicate that federated fine-tuning of generative language models is both feasible and effective for privacy-sensitive medical applications.
